...
@Component({
  selector: 'app-kycdilogform',
  templateUrl: './kycdilogform.component.html',
  styleUrls: ['./kycdilogform.component.scss']
})
export class KycdilogformComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    public dialogRef: MatDialogRef<KycdilogformComponent>,) { }
    ...
    this.apiHandlerService.apiGet(API.QUEST_ENDPOINTS.GET_BUSINESS_PROFILE).pipe().subscribe(
      {
        next: data => {
          if (data.data) {
            if (data.data.reseller_profile.business_name) {
              this.business_name = data.data.reseller_profile.business_name;
  ...
  handleSubmit() {
   //  console.log(this.addressForm.value.user_group);
  ...
  handleSubmitSecond() {
    ...
    uploadData2.append('user_group',this.addressForm.value.user_group)
    this.apiHandlerService.apiPost(API.QUEST_ENDPOINTS.UPLOAD_BUISNESSKYC, uploadData2, {},
      { contentType: { isFormDataContent: true } }).subscribe({
        ...
        error: err => {
          // this.toasterService.Error("GST number already exists");
          this.isLoginProcessing = false;
          this.dialogRef.close();
        },
        complete: () => {
